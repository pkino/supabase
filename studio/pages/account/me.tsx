...
const DeleteUserButton = observer(() => {
  ...
  const onConfirmDelete = async (values: any, { setSubmitting }: any) => {
    ...
    if (response.error) {
      let errorMessage = `Failed to delete user: ${response.error.message}`
      if (response.error.code === 403) {
        errorMessage = `You must remove yourself, transfer ownership or delete these projects before you can delete your user. ${response.error.message}`
      }
      ui.setNotification({
        category: 'error',
